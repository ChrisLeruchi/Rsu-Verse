...
import { Send, Bookmark, Repeat, Repeat1, MessageCircle, ArrowBigDown, ArrowBigUp, ArrowUpRight, MoreHorizontal } from "lucide-react";
import { formatRelativeTime } from "./formatRelativeTime";

export function PostDetail({ posts, handleSave, handleRepost, handleDownvotes, handleUpvote, getVerseIcon }) {
  ...
  if (!post) {
    return (
      <div className="w-full max-w-md mx-auto flex items-center justify-center min-h-screen bg-ink text-white/30 text-[14px] font-light tracking-wide">
        Loading campus thread...
      </div>
    );
  }
    ...
    <>
      ...
      >
        ...
        <div>
          {post.content?.images && post.content.images.length > 0 && (
            <div className={`mt-3 grid gap-1.5 overflow-hidden bomorerder border-white/5 ${post.content.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2'
              }`}>
          ...
          <div className="flex items-center justify-between">
            <div
              ...
              <button
                ...
                className=
                {`flex gap-1 flex-1 items-center relative h-8 transition-colors select-none group outline-none justify-center ${post.userInteraction?.voteStatus === 'down' ? 'text-rose' : 'hover: text-rose/80 text-white/70'
                  }`}>
      ...
      <div className="flex flex-col gap-4 pt-5">
        {post.engagement.comments.length === 0 ? (
          <div className="flex justify-center items-center text-white/40 px-3">
            No comments yet
          </div>
        ) : (
          post.engagement.comments.map(comment => (
            <div
              key={comment.id}
              className="flex gap-3 border-b border-white/5 pb-15 px-3"
            >
              ...
              <div className="flex flex-1 flex-col gap-4">
                ...
                <div className="flex justify-between items-center text-white/70">
                  ...
                  <button>
                    ...
                    />
                    <span>
                      {comment.engagement.downvotes === 0
                        ? ''
